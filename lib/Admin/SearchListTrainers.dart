...
import 'package:flutter_application_2/Admin/TrainersDetailsAdmin.dart';
...
class SearchListTrainers extends StatefulWidget {
  const SearchListTrainers({Key? key}) : super(key: key);

  @override
  _SearchListState createState() => _SearchListState();
}

class _SearchListState extends State<SearchListTrainers> {
                ...
                if (index == 0) {
                  ...
                  return Container(
                    ...
                    backgroundImage: image!=""? MemoryImage(base64Decode(image)):AssetImage('imagess/profilePic.png') as ImageProvider,
                        ...
                        onTap: () {
                          global.trainerUsername=_list.elementAt(index-1);
                          Navigator.push(
              context,
              PageTransition(
                type: PageTransitionType.leftToRightWithFade,
                child: TrainersDetailsAdmin(),
...
'name':item,
 ...
 var res = await http.post(Uri.parse(global.globall + "/users/deleteTrainersFromAdmin"),
            headers: {
              'Content-Type': 'application/json; charset=UTF-8',
              'token':global.token
            },
            ...
            if(res.statusCode == 200) {
               Navigator.push(
          context,
          MaterialPageRoute(builder: (context) => ShowTrainers()),
        );
