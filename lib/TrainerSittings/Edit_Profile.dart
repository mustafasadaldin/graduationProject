...
class Edit_ProfileTrainer extends StatefulWidget {
  @override
...
class Edit_ProfileState extends State<Edit_ProfileTrainer> {
...
Widget Waitforme() {
  ...
  Future Upload(File imageFile) async {
    ...
    var uri = Uri.parse(global.globall + "/trianers/image");
    ...
    Navigator.push(
      context,
      MaterialPageRoute(builder: (context) => Edit_ProfileTrainer()),
    );
  }
  ...
  Future image2() async {
    ...
    var res = await http.post(Uri.parse(global.globall + "/trianers/image1"),
        headers: {
          'Content-Type': 'application/json; charset=UTF-8',
        },
        body: body1);
  ...
  Future<void> changeInfo() async {
    if (username.text.isEmpty &&
      ...
      return _displayErrorMotionToastuserNameInvalid(
          "at least one field must be filled !");
    }
    if (username.text.isNotEmpty) {
      var body1 = jsonEncode({"username": username.text});

      var res = await http.post(Uri.parse(global.globall + "/trainer/info"),
          headers: {
            'Content-Type': 'application/json; charset=UTF-8',
            'token': global.token
          },
  ...
  print(res.statusCode);
      if (res.statusCode == 400) {
        if (res.body == "invalid username") {
          return _displayErrorMotionToastuserNameInvalid(
              "you must enter your full Quadruple name");
        } else if (res.body == "not unique username") {
          return _displayErrorMotionToastuserNameInvalid(
              "This name is already used by other users!");
        }
      }
    ...
    if (age.text.isNotEmpty) {
      var body1 = jsonEncode({"age": age.text});
    ...
    if (experience.text.isNotEmpty) {
      var body1 = jsonEncode({"excperience": experience.text});
      ...
      if (res.statusCode == 400) {
        return _displayErrorMotionToastuserNameInvalid(
            "Error in changing height");
      }
      global.experience = experience.text.toString();
    }
